...
%     perfmap_range_phiz(perfmap_range_phiz> 2*pi)= 2*pi;
    ...
    if dbg_load_perfmap
      ...
      if ~pm_loaded && ~isempty(matfilelist_pm)
        cds_log(-1, sprintf(['[constraints_traj] Konfig %d/%d: Keine der %d ' ...
          'gespeicherten Redundanzkarten passen'], Structure.config_index, ...
          Structure.config_number, length(matfilelist_pm)));
      end
    end
    if ~pm_loaded
      set_perfmap = struct('settings_ik', s, ...
        ...
        'maplim_phi', perfmap_range_phiz, 'verbose', true);
      [H_all, Q_all, s_ref, s_tref, phiz_range] = R.perfmap_taskred_ik( ...
        Traj_0.X(1:nt_red,:), Traj_0.IE(Traj_0.IE~=0), set_perfmap);
      ...
      save(matfile_pm, 'Structure', 'H_all', 'Q_all', 's_ref', 's_tref', ...
...
if Structure.task_red && Set.general.taskred_dynprog && ...
  ...
  s_dp = struct(...
    ...
    'cost_criterion', 'ik_criterion', ... % Standardwert, kann überschrieben werden.
  ...
  cds_log(2, sprintf(['[constraints_traj] Konfig %d/%d: %1.1fs für DP. ', ...
    ...
    'reiche Übergänge. %s(%s)=%1.5f'], Structure.config_index, Structure.config_number, ...
    toc(t1), DPstats.nt_ik, DPstats.nt_ik/length(Traj_0.t), ...
    DPstats.n_statechange_succ, DPstats.n_statechange_total, ...
    s_dp.cost_mode, s_dp.cost_criterion, DPstats.F_opt_stage(end)));
  ...
  if Set.general.debug_calc && strcmp(s_dp.cost_criterion, 'actforce') && ...
      strcmp(s_dp.cost_mode, 'max') % Prüfe ob bei Kraft als Kriterium richtig gerechnet wurde
    % Prüfe ob die Antriebskraft in der DP genauso ist wie hier neu berechnet
    Traj_0_test = Traj_0;
    Traj_0_test.X(:,6) = TrajDetailDP.X6;
    ...
    if any(abs(tau_a_max_cds(:)-DPstats.F_opt_stage(end))>1e-6)
      ...
      error(['Die Antriebskräfte in der DP stimmen nicht mit den ' ...
        'Maßsynthese-Funktionen überein']);
    end
  end
...
if Structure.task_red && (any(corrQD < 0.95) || any(corrQ < 0.98)) || ...
    ~Structure.task_red && any(q_maxstep_revjoint>10*pi/180) % TODO: Entstehung dieser Zeile unklar.
