...
%   1e13...1e14: Parameter unplausibel
% physval
...
% Plausibilitätsprüfung der Parameter. Manche sind schon ungültig ohne
% Berechnung der IK
if R.Type == 2 && any(R.DesPar.platform_method == [4 5 6]) && ... % PKM, Plattform paarweise
  ...
  if r_eff_exc_rel > 1 % obere Grenze überschritten durch zu großen Paarabstand
    fval(:) = 1e13 * (1 + 9 * 2/pi*atan(r_eff_exc_rel-1)); % 1->1e13, 5->8.6e13
    cds_log(2,sprintf(['[fitness] G=%d;I=%d. Fitness-Evaluation in %1.2fs. ', ...
      'fval=%1.3e. Paarabstand ist um %1.1f%% zu groß. Effektiver Basis-', ...
