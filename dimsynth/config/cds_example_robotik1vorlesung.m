...
% Der Roboter soll eher mittig im Arbeitsraum sein (und nicht in Aufgabe)
Set.optimization.basepos_limits = [[-0.8, 0]; [-0.8, 0.8]; [0 1]];
...
% Kollisionskörper unter die Trajektorie setzen. Entspricht z.B. Tisch.
Set.task.obstacles = struct( ...
  'type', uint8(3), ... % Kapsel (nichts anderes implementiert)
  'params', [[mean(minmax2(Traj.X(:,1:2)')') 0.3], ...
             [mean(minmax2(Traj.X(:,1:2)')') min(Traj.X(:,3))-0.35], 0.3]); %#ok<UDIM>
...
Set.general.plot_details_in_fitness = 0*4e9; % debug: Wert setzen
Set.general.plot_robot_in_fitness = 0e3; % debug: Wert setzen
...
Set.general.maxduration_animation = 10; % Länge des Videos begrenzen
Set.general.eval_figures = {'pareto_all_phys', 'robvisuanim'};
% Debug: Auswahl der Strukturen
% Set.structures.whitelist = {'S6RRRRRR10V3'}; % Industrieroboter-Kinematik
...
% Debug: 5FG-Roboter ausschließen
...
if cluster
  ...
  Set.general.computing_cluster_max_time = 3600*0.5; % eine Stunde
  Set.general.computing_cluster_cores = 8; % Bei 16 evtl Probleme bei Videoerstellung
end
