...
if length(intersect(Set.optimization.objective_ik, {'default', 'none', 'constant', ...
    'ikjac_cond', 'jac_cond', 'coll_par', 'instspc_par'})) ~= length(Set.optimization.objective_ik)
...
if isfield(Set.task.payload.If) && any(~isnan(Set.task.payload.If)) && any(~isnan(Set.task.payload.If))
...
if Set.general.computing_cluster
  ...
  for kk = 1:length(I1_Struct)
    ...
    jobIDs(1,kk) = jobStart(struct( ...
      ...
      'mem', ceil(16+2.5*ppn), ... % Hergeleitet aus MaxRSS-Wert aus diversen Job-Status-Emails
    ...
    jobIDs(2,kk) = jobStart(struct( ...
      ...
      'mem', ceil(16+2.5*ppn), ... % gleiche Zahl wie oben (da beim Finish-Job die Fitness-Funktion auch einmal ausgeführt wird)
