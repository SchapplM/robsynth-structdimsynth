...
if length(intersect(Set.optimization.objective, {'mass', 'energy', 'power', ...
    ...
    'positionerror', 'actvelo', 'chainlength', 'installspace', 'footprint', 'colldist', 'mrk1', 'mrk2', 'mrk3', 'mrk4'})) ~= ...
...
eval_figures_allowed = {'robvisu', 'robvisuanim', 'animation', 'jointtraj', ...
  'pareto', 'pareto_dimsynth_desopt', 'pareto_desopt', 'dynamics', ...
  'dynparvisu', 'optpar', 'springrestpos', ... % cds_vis_results_figures
  'pareto_all_phys', 'pareto_all_fval', 'pareto_all_phys_groups', ...
  'pareto_all_fval_groups', 'histogram', 'fitness_various'}; % cds_vis_results
...
if Set.general.computing_cluster
  ...
  for kk = 1:length(I1_Struct)
    ...
    cds_log(1, sprintf(['Berechnung von %d Robotern wurde auf Cluster hochgeladen ', ...
      '(Part %d: %d-%d). Ende. Die Ergebnisse müssen nach Beendigung der ', ...
      'Rechnung manuell heruntergeladen werden.'], length(I1_kk:I2_kk), kk, I1_kk, I2_kk));
...
if ~isempty(Set.structures.whitelist)
  ...
  if ~isempty(missing_in_robotlist)
    if Set.structures.whitelist_overrules_filters
      cds_log(-1, sprintf(['Es wurde eine Positiv-Liste mit %d Einträgen ', ...
      ...
      cds_log(2, sprintf(['Es wurde eine Positiv-Liste mit %d Einträgen ', ...
        'übergeben. Durch whitelist_overrules_filters=0 wurden nur %d dieser ' ...
        'Strukturen wurden gewählt. %d fehlen: %s'], ...
        length(Set.structures.whitelist), length(Names_in_Struct{i}), ...
        length(unique(missing_in_robotlist)), disp_array(missing_in_robotlist, '%s')) );
    end
  end
