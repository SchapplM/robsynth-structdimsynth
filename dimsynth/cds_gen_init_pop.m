...
%     disp_array(Structure.varnames(missing_file_in_local), '%s'));
  ...
  if Structure_i.Type == 2 && any(Structure_i.Coupling(1) == [4 8]) && ...% Konische Gestellgelenke
     ...
     Set.optimization.min_inclination_inclined_platform_joint > 0 && ... % Einstellung gesetzt
     (~isfield(Set_i.optimization, 'min_inclination_inclined_platform_joint') ||... % geladene Einstellung nicht so streng
     Set_i.optimization.min_inclination_inclined_platform_joint<Set.optimization.min_inclination_inclined_platform_joint) ...
